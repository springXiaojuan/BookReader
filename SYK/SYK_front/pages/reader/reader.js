...
Page({
  ...
  getBookSources: function (book_id) {
    wx.request({
      url: api.bookSources(book_id),
      success: res => {
        this.setData({
          bookSources: res.data
        });
        console.log(this.data.bookSources);
  ...
  getBookChapters: function (source_id) {
    wx.request({
      url: api.bookChapters(source_id),
      success: res => {
        this.setData({
  ...
  getChapterContent: function (link) {
    ...
    wx.request({
      ...
      success: res => {
        ...
        //使用Wxparse格式化小说内容,对于收费的内容暂时不显示
        WxParse.wxParse('article', 'html', this.data.indexChapterContent.chapter.cpContent ? '此书尚未开放,去看看别的吧!' : this.data.indexChapterContent.chapter.body, this);
        //等到渲染页面后调节scrollTop
        this.setData({
