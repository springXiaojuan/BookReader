...
Page({
  ...
  addShelf: function () {
    ...
    console.log("this.data.addedShelf = " + this.data.addedShelf);
  ...
  getRelatedRecommendedBooks: function (book_id) {
    ...
    wx.request({
      url: api.relatedRecommendedBooks(book_id),
      success: res => {
        console.log(res);
        this.setData({
  ...
  randomRecommendBooks: function () {  //在所有推荐书籍中随机选出三本展示
    ...
    console.log(this.data.allRecommendBooks);
    let randomIndex;
    for (let i = 0; i < 3; i++) {
